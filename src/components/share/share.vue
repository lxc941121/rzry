...
export default {
  ...
  methods: {
    ...
    tojoin(){
      ...
      {
        this.$router.push({ path: '/apply', query: { projectItem: this.projectinfo.id } });
      }else{
    ...
    {
        const self = this;
        self.$axios.post("/api/sys/yl/project/info?id="+proid,"", {
            headers: {
                "token":  window.sessionStorage.getItem('token')
            }
        }).then( res =>{
    ...
    getorderinfo(id){
        const self = this;
        self.$axios.post("/api/sys/yl/order/info?id="+id,"", {
            headers: {
                "token":  window.sessionStorage.getItem('token')
            }
        }).then( res =>{
